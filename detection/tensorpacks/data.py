...
from detection.tensorpacks.utils.generate_anchors import generate_anchors
from detection.tensorpacks.utils.np_box_ops import area as np_area
...
def get_all_anchors(stride=None, sizes=None):
    ...
    cell_anchors = generate_anchors(
        ...
        ratios=np.array(cfg.RPN.ANCHOR_RATIOS, dtype=np.float))  # 5
    # anchors are intbox here.
    # anchors at featuremap [0,0] are centered at fpcoor (8,8) (half of stride)

    max_size = cfg.PREPROC.MAX_SIZE  # 1333
...
def get_anchor_labels(anchors, gt_boxes, crowd_boxes):
    ...
    # get the max iou and its indices
...
def get_attributes_dataflow(augment=False, two_cls=False):
    ...
    roidbs_train = load_many('/root/datasets/WiderAttribute', 'train', augment, two_cls)
    roidbs_val = load_many('/root/datasets/WiderAttribute', 'val', augment, two_cls)
