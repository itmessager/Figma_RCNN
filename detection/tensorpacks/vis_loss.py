...
def detect_one_image2(img, model_func):
    ...
    boxes, probs, labels, *masks, male = model_func(resized_img,np.array([0,1,0,1,0,0,1,1,1,0,0]))
    boxes = boxes / scale
    ...
    results = [DetectionResult(*args) for args in zip(boxes, probs, labels, masks, male)]
    return results
...
if __name__ == '__main__':
    ...
    if args.predict:
        ...
        pred = OfflinePredictor(PredictConfig(
            ...
            output_names=['output/boxes', 'output/scores', 'output/labels', 'output/masks', 'male_loss']))
