...
class ResNetC4Model(DetectionModel):
    ...
    def build_graph(self, *inputs):
        ...
        is_training = True #get_current_tower_context().is_training
        # build resnet c4
        featuremap = resnet_c4_backbone(image, cfg.BACKBONE.RESNET_NUM_BLOCK[:3])
        # predict attrs branch
        if is_training:

            boxes_on_featuremap = inputs['gt_boxes'] * (1.0 / cfg.RPN.ANCHOR_STRIDE)  # ANCHOR_STRIDE = 16
            roi_resized = roi_align(featuremap, boxes_on_featuremap, 14)
            feature_attrs = resnet_conv5(roi_resized, cfg.BACKBONE.RESNET_NUM_BLOCK[-1])  # nxcx7x7 # RESNET_NUM_BLOCK = [3, 4, 6, 3]
...
def visualize(model, model_path, nr_visualize=100, output_dir='output'):
    ...
    with tqdm.tqdm(total=nr_visualize) as pbar:
        for idx, dp in itertools.islice(enumerate(df), nr_visualize):
            ...
            final_boxes, final_scores, final_labels, male = pred(img, gt_boxes, gt_labels)
            ...
            results = [DetectionResult(*args) for args in
                       zip(final_boxes, final_scores, final_labels, male,
...
if __name__ == '__main__':
    ...
    if args.visualize or args.evaluate or args.predict:
        ...
        else:  # args.predict, only need input images
            pred = OfflinePredictor(PredictConfig(
                model=MODEL,  # model
                session_init=get_model_loader(args.load), # weight
                input_names=['image'],
                output_names=['output/boxes', 'output/scores', 'output/labels', 'output/masks', 'male/output']))
            if args.evaluate:
